...
const Reconciliation = () => {
  ...
  const invoiceCols = [{ title: '发票ID', dataIndex: 'invoiceId' }, { title: '发票金额', dataIndex: 'invoiceMoney', render: (v: number) => `¥${v?.toFixed(2)}` }, { title: '运费', dataIndex: 'shippingFee' }, { title: '总金额', dataIndex: 'totalMoney', render: (v: number) => `¥${v?.toFixed(2)}` }, { title: '开票时间', dataIndex: 'invoiceTime' }];
  const paymentCols = [{ title: '付款金额', dataIndex: 'payment', render: (v: number) => `¥${v?.toFixed(2)}` }, { title: '付款时间', dataIndex: 'paymentTime' }, { title: '备注', dataIndex: 'comment' }];
  const penaltyCols = [{ title: '罚款金额', dataIndex: 'penalty', render: (v: number) => `¥${v?.toFixed(2)}` }, { title: '罚款时间', dataIndex: 'penaltyTime' }, { title: '备注', dataIndex: 'comment' }];
    ...
    <div>
      <Card style={{ marginBottom: 16 }}>
        <Row gutter={16} align="middle">
          ...
          <Col>日期范围:</Col>
          <Col><DatePicker.RangePicker onChange={setDateRange} /></Col>
